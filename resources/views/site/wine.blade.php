...
@section('container')
    <div class="container">
        <div id="what" class="container__main-img " style="background: url({{ asset($activeMenu['image'] ?? '') }}) no-repeat; background-size: cover;background-position: center center;">
            <div class="container__main-title js-scroll">@lang('site.wineList.title')</div>
            {{--<div class="container__scroll-btn js-scroll"></div>--}}
        </div>
        <span class="container__title">
            @lang('site.wineList.title')
        </span>
            ...
            <div class="category">
                @if($items)
                    @foreach($items as $item)
                        ...
                        <a href="{{ route('wineGroup', ['group' => $item['name']]) }}" class="category__item" style="background: url('{{ asset($item['preview_image'] ?? '') }}'); background-size: cover;">
