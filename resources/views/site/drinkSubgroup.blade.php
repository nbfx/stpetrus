...
@section('container')
    <div class="container">
        ...
        <div class="category__wrapper">
            <div class="category">
                {{--TODO: картинка не выводится, короч. она приходит - dump(asset($items['image'])), но не выводится--}}
                ...
                @if($items && !empty($items['children']))
                    @foreach($items['children'] as $child)
                        <div class="category__sub-items">
                            {{--TODO: всё в одну строку О_о --}}
                            <span class="category__dish">{{ $child['title'] }}/ {{ config('app.currency').$child['price'] }}</span>
